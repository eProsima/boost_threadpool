...
namespace boost {
    namespace threadpool {
        namespace detail {
            ...
            */
            template<
                    template<template<typename> class Task = task_functor, typename TaskReturnType = void> class SchedulingPolicy,
            ...
            >
            class pool_core
                    : public enable_shared_from_this<pool_core<SchedulingPolicy, SizePolicy, SizePolicyController, ShutdownPolicy, Task>>,
                      private noncopyable {
            ...
            public: // Type definitions
                ...
                typedef pool_core<SchedulingPolicy,
                        SizePolicy,
                        SizePolicyController,
                        ShutdownPolicy, Task> pool_type;           //!< Indicates the thread pool's type.
...
//                BOOST_STATIC_ASSERT(is_void<typename result_of<task_type()>::type>::value);
            ...
            public:
                /// Constructor.
                pool_core()
                        : m_worker_count(0), m_target_worker_count(0),
...
// TODO is only called once
                ...
                */
                ...
                template<typename TaskReturnTypename>
                bool schedule(task_type<TaskReturnTypename> const &task) volatile {
                ...
                */
                bool wait(xtime const &timestamp, size_t const task_threshold = 0) const volatile {
                    ...
                    if (0 == task_threshold) {
                        while (0 != self->m_active_worker_count ||
                               !self->m_scheduler.empty()) {
                            if (!self->m_worker_idle_or_terminated_event.timed_wait(lock, timestamp)) {
                                return false;
                            }
                        }
                    } else {
            ...
            private:
                ...
                void terminate_all_workers(bool const wait) volatile {
                    pool_type *self = const_cast<pool_type *>(this);
                    {
                        ...
                        self->m_destruct_barrier.reset(new barrier(
                                m_target_worker_count + 1));
                ...
                */
                bool resize(size_t const worker_count) volatile {
                    ...
                    if (m_worker_count <=
                        m_target_worker_count) { // increase worker count
